...
def get_mongodb_connection():
    ...
    Priority: MONGODB_URI env var > st.secrets[MONGODB_URI] > api.txt > hardcoded URI
    """
    try:
        ...
        if not mongo_uri:
            ...
            for p in candidates:
                if p.exists():
                    for line in p.read_text(encoding="utf-8").splitlines():
                        if ("MONGODB_URI" in line or "mongo_uri" in line) and "=" in line:
